...
func TestSecretMaterializationPlanAcceptsLegacyEmptyAllowlistDigest(t *testing.T) {
	plan, err := CompileSecretMaterializationPlan("tenant", "project", "environment", "revision", "sha256:template", "target", nil, "sha256:input", time.Unix(1, 0))
	if err != nil {
		t.Fatal(err)
	}
	legacy, err := plan.legacyCanonicalDigest()
	if err != nil {
		t.Fatal(err)
	}
	plan.Digest = legacy
	...
	if err := transported.Validate(); err != nil {
		t.Fatalf("legacy plan must remain valid after JSON transport: %v", err)
	}
}
